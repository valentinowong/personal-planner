-- Allow collaborators to write tasks on shared lists and surface them in get_tasks_window.

-- Recreate task write policies to include collaborators on shared lists.
...
create policy tasks_insert_shared on public.tasks
  for insert
  with check (
    (user_id = auth.uid() and list_id is null)
    or (list_id is not null and list_shared_with_user(list_id, auth.uid()))
  );

create policy tasks_update_shared on public.tasks
  for update
  using (
    user_id = auth.uid()
    or (list_id is not null and list_shared_with_user(list_id, auth.uid()))
...
create policy tasks_delete_shared on public.tasks
  for delete
  using (
    user_id = auth.uid()
    or (list_id is not null and list_shared_with_user(list_id, auth.uid()))
...
-- Update get_tasks_window to return tasks on shared lists.

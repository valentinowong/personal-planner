...
import { formatHourLabel, HOURS, HOUR_BLOCK_HEIGHT, CALENDAR_DAY_WIDTH } from "./time";
  ...
  const updateExternalHover = useCallback(
    (target: PlannerDropTarget | null, coords?: { x: number; y: number }, task?: LocalTask) => {
      const isCalendarTarget =
        target && (target.type === "calendarSlot" || (target.type === "day" && target.origin !== "daily"));
      const internalCalendarHover = Boolean(isCalendarTarget && internalDragRef.current);
      const dropTarget = internalCalendarHover ? null : target;
      hoverTargetRef.current = dropTarget;

      if (!dropTarget) {
        onDayHoverChange?.(null);
        onListHoverChange?.(null);
        if (!internalCalendarHover && coords && task) {
          ...
          {days.map((day) => {
            ...
            return (
              ...
              >
                ...
                <View
                  style={[styles.calendarDayTasks, { height: HOURS.length * HOUR_BLOCK_HEIGHT }]}
